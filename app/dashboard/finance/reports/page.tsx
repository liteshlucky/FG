                                        ...
                                        <table className="min-w-full divide-y divide-slate-700">
                                            <thead className="bg-slate-900/50">
                                                <tr>
                                                    ...
                                                    <th className="px-5 py-3 text-center text-xs font-medium uppercase tracking-wider text-slate-400">Target</th>
                                                    <th className="px-5 py-3 text-right text-xs font-medium uppercase tracking-wider text-slate-400">PT Revenue</th>
                                                    <th className="px-5 py-3 text-right text-xs font-medium uppercase tracking-wider text-slate-400">Est. Incentive</th>
                                                </tr>
                                            </thead>
                                                        ...
                                                        <React.Fragment key={trainer.id}>
                                                            ...
                                                            >
                                                                <td className="px-5 py-4 whitespace-nowrap">
                                                                    <span className={`inline-flex items-center justify-center h-8 w-8 rounded-full font-bold text-sm ${index === 0 ? 'bg-yellow-500/20 text-yellow-500 border border-yellow-500/30' :
                                                                ...
                                                                <td className="px-5 py-4 whitespace-nowrap text-center">
                                                                    {trainer.targetMet ? (
                                                                        <div className="flex items-center justify-center text-emerald-500 bg-emerald-500/10 rounded-full w-8 h-8 mx-auto" title="Target Met">
                                                                            <CheckCircle2 className="h-5 w-5" />
                                                                        </div>
                                                                    ) : (
                                                                        <div className="flex items-center justify-center text-slate-600 bg-slate-800 rounded-full w-8 h-8 mx-auto" title={`Progress: ${trainer.ptCount}/${trainer.targetGoal}`}>
                                                                ...
                                                                <td className="px-5 py-4 whitespace-nowrap text-sm font-bold text-emerald-400 text-right">
                                                                    <div className="flex flex-col items-end">
                                                                        <span>{formatCurrency(trainer.incentive)}</span>
                                                                        <span className="text-[10px] text-slate-500 font-normal">
                                                                            {trainer.ptCount > 7 ? '50% Tier' : '40% Tier'}
                                                                        </span>
                                                                    </div>
                                                                </td>
                                                            </tr>
                                                            ...
                                                            {expandedTrainers[trainer.id] && (
                                                                <tr>
                                                                    <td colSpan={6} className="bg-slate-900/50 p-0 border-t border-slate-700/50 w-full">
                                                                        <div className="px-12 py-5 w-full">
                                                                            <h4 className="text-sm font-semibold text-slate-400 mb-3">PT Member Breakdown & Justification</h4>
                                                                            {trainer.clients && trainer.clients.length > 0 ? (
                                                                                <div className="bg-slate-800 border border-slate-700 rounded-lg overflow-hidden">
                                                                                    <table className="min-w-full divide-y divide-slate-700">
                                                                                        <thead className="bg-slate-900/50">
                                                                                            <tr>
                                                                                                <th className="px-4 py-2 text-left text-[10px] font-medium text-slate-500 uppercase tracking-wider">Member</th>
                                                                                                <th className="px-4 py-2 text-left text-[10px] font-medium text-slate-500 uppercase tracking-wider">Paid On</th>
                                                                                                <th className="px-4 py-2 text-right text-[10px] font-medium text-slate-500 uppercase tracking-wider">Plan (Months)</th>
                                                                                                <th className="px-4 py-2 text-right text-[10px] font-medium text-slate-500 uppercase tracking-wider">Monthly Rev</th>
                                                                                                <th className="px-4 py-2 text-center text-[10px] font-medium text-slate-500 uppercase tracking-wider">Tier Rate</th>
                                                                                                <th className="px-4 py-2 text-right text-[10px] font-medium text-slate-500 uppercase tracking-wider">Incentive</th>
                                                                                            </tr>
                                                                                        </thead>
                                                                                        <tbody className="divide-y divide-slate-700/50">
                                                                                            {trainer.clients.map((client: any, cIdx: number) => (
                                                                                                <tr key={client.id || cIdx} className="hover:bg-slate-700/30 transition-colors">
                                                                                                    <td className="px-4 py-2 hover:bg-slate-700/50 transition-colors">
                                                                                                        <Link href={`/dashboard/members/${client.memberId || client.id}`} className="block">
                                                                                                            <div className="text-xs font-medium text-blue-400 hover:text-blue-300 flex items-center gap-1">
                                                                                                                {client.name}
                                                                                                                <ArrowUpRight className="h-3 w-3" />
                                                                                                            </div>
                                                                                                            <div className="text-[10px] text-slate-500">{client.memberId}</div>
                                                                                                        </Link>
                                                                                                    </td>
                                                                                                    <td className="px-4 py-2 text-[10px] text-slate-400">
                                                                                                        {new Date(client.paymentDate).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit' })}
                                                                                                    </td>
                                                                                                    <td className="px-4 py-2 text-right">
                                                                                                        <div className="text-xs text-slate-300">₹{client.ptTotalPlanPrice?.toLocaleString()}</div>
                                                                                                        <div className="text-[10px] text-slate-500">({client.monthsDuration} months)</div>
                                                                                                    </td>
                                                                                                    <td className="px-4 py-2 text-right text-xs font-semibold text-slate-200">
                                                                                                        {formatCurrency(client.monthlyRevenue)}
                                                                                                    </td>
                                                                                                    <td className="px-4 py-2 text-center">
                                                                                                        <span className={clsx(
                                                                                                            "text-[10px] font-bold px-1.5 py-0.5 rounded",
                                                                                                            client.rate === 50 ? "bg-purple-500/10 text-purple-400 border border-purple-500/20" : "bg-blue-500/10 text-blue-400 border border-blue-500/20"
                                                                                                        )}>
                                                                                                            {client.rate}%
                                                                                                        </span>
                                                                                                    </td>
                                                                                                    <td className="px-4 py-2 text-right text-xs font-bold text-emerald-400">
                                                                                                        {formatCurrency(client.incentive)}
                                                                                                    </td>
                                                                                                </tr>
                                                                                            ))}
                                                                                            <tr className="bg-slate-900/50 border-t-2 border-slate-700">
                                                                                                <td colSpan={2} className="px-4 py-2 text-xs font-bold text-slate-400 uppercase">Total</td>
                                                                                                <td className="px-4 py-2 text-right text-xs font-bold text-slate-200">{formatCurrency(trainer.revenue)}</td>
                                                                                                <td></td>
                                                                                                <td className="px-4 py-2 text-right text-sm font-bold text-emerald-400">{formatCurrency(trainer.incentive)}</td>
                                                                                            </tr>
                                                                                        </tbody>
                                                                                    </table>
                                                                                </div>
