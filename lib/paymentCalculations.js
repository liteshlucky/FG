...
export async function updateMemberPaymentStatus(memberId, MemberModel, PaymentModel) {
    try {
        ...
        const ptPayments = payments.filter(p => p.planType === 'PTplan' || p.planType === 'pt_plan');
        const membershipPayments = payments.filter(p => p.planType !== 'PTplan' && p.planType !== 'pt_plan');

        const membershipTotalPaid = membershipPayments.reduce((sum, p) => sum + (p.amount || 0), 0);
        const ptTotalPaid = ptPayments.reduce((sum, p) => sum + (p.amount || 0), 0);
        ...
        member.totalPaid = membershipTotalPaid;
        ...
        member.ptPaymentStatus = calculatePaymentStatus(
            ...
            ptTotalPaid,
            0 // usually no separate admission fee just for PT
        );
